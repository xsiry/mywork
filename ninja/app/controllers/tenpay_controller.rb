...
class TenpayController < ApplicationController
  ...
  def pay
    ...
    tenpay = merchants["merchants"]["tenpay"]
    auth_id   = tenpay["auth_id"]
    auth_key  = tenpay["auth_key"]
    ...
    tenpay_params['partner'] = auth_id                                   #商户号
    ...
    tenpay_params['total_fee'] = (params[:price].to_i * 100) rescue 0               #商品金额,以分为单位
    ...
    md5_sign = tenpay_sign(params_str + "&key=#{auth_key}")
